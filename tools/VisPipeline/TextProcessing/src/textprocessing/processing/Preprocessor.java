...
public class Preprocessor {
    ...
    public AbstractMatrix getMatrix(int lowerCut, int upperCut, int numberGrams,
            StemmerType stemmer) throws IOException {
    ...
    public AbstractMatrix getMatrixSelected(int lowerCut, int upperCut, int numberGrams,
            StemmerType stemmer, ArrayList<AbstractInstance> selected) throws IOException {
    ...
    private AbstractMatrix getMatrix(ArrayList<Integer> ids) throws IOException {
        ...
        AbstractMatrix matrix = new SparseMatrix();
        ...
        for (int i = 0; i < ids.size(); i++) {
            ...
            SparseVector spv = new SparseVector(vector, ids.get(i), corpus.getClassData()[i]);
            matrix.addRow(spv);
        }
    ...
    public static void main(String[] args) {
        try {
            Corpus corpus = new ZipCorpus("D:\\Meus documentos\\FERNANDO\\Corpora\\cbr-ilp-ir-son.zip", 1);
            Preprocessor preproc = new Preprocessor(corpus);
            AbstractMatrix matrix = preproc.getMatrix(35, -1, 1, StemmerType.PORTER);

            matrix.save("D:\\Meus documentos\\FERNANDO\\test.data");
