...
public class SilhouetteCoefficientView extends javax.swing.JDialog {
    ...
    public void display(Projection3DModel model, Scalar scalar) throws IOException {
        this.model = model;
        this.matrix = Projection3DUtil.modelToMatrix(model, scalar);
    ...
    private void createScalars(float[] silhouette) throws IOException {
        ...
        Scalar silh = model.addScalar(ProjectionConstants.SILHOUETTE);
        Scalar nsilh = model.addScalar(ProjectionConstants.NORM_SILHOUETTE);
        Scalar clsilh = model.addScalar(ProjectionConstants.CLUSTER_SILHOUETTE);
        Scalar nclsilh = model.addScalar(ProjectionConstants.NORM_CLUSTER_SILHOUETTE);
        ...
        for (int i = 0; i < instances.size(); i++) {
            ...
            pi.setScalarValue(silh, silhouette[i]);
            pi.setScalarValue(nsilh, normsilhouette[i]);
            pi.setScalarValue(clsilh, clsilhouette[i]);
            pi.setScalarValue(nclsilh, normclsilhouette[i]);
        }
    }
