...
public class ForceScheme {
    ...
    public float iteration(DistanceMatrix dmat, AbstractMatrix projection) {
        float error = 0.0f;

        if (projection.getRow(0).size() == 2) {
            //for each instance
            for (int ins1 = 0; ins1 < projection.getRowCount(); ins1++) {
                int instance = this.index[ins1];

                //for each other instance
                for (int ins2 = 0; ins2 < projection.getRowCount(); ins2++) {
                    ...
                    float x1x2 = (projection.getRow(instance2).getValue(0) - projection.getRow(instance).getValue(0));
                    float y1y2 = (projection.getRow(instance2).getValue(1) - projection.getRow(instance).getValue(1));
...
//                delta = (float) Math.sqrt(Math.abs(delta));
                    ...
                    //moving ins2 -> ins1
                    projection.getRow(instance2).setValue(0,projection.getRow(instance2).getValue(0) +  delta * (x1x2 / dr2));
                    projection.getRow(instance2).setValue(1,projection.getRow(instance2).getValue(1) +  delta * (y1y2 / dr2));
                }
            }

            error /= (projection.getRowCount() * projection.getRowCount()) - projection.getRowCount();
        } else if (projection.getDimensions() == 3) {
            //for each instance
            for (int ins1 = 0; ins1 < projection.getRowCount(); ins1++) {
                int instance = this.index[ins1];
                ...
                for (int ins2 = 0; ins2 < projection.getRowCount(); ins2++) {
                    ...
                    float x1x2 = (projection.getRow(instance2).getValue(0) - projection.getRow(instance).getValue(0));
                    float y1y2 = (projection.getRow(instance2).getValue(1) - projection.getRow(instance).getValue(1));
                    float z1z2 = (projection.getRow(instance2).getValue(2) - projection.getRow(instance).getValue(2));
...
//                delta = (float) Math.sqrt(Math.abs(delta));
                    ...
                    //moving ins2 -> ins1
                    projection.getRow(instance2).setValue(0,projection.getRow(instance2).getValue(0) +  delta * (x1x2 / dr3));
                    projection.getRow(instance2).setValue(1,projection.getRow(instance2).getValue(1) +  delta * (y1y2 / dr3));
                    projection.getRow(instance2).setValue(2,projection.getRow(instance2).getValue(2) +  delta * (z1z2 / dr3));
                }
            }
    ...
    public float iteration(AbstractMatrix matrix, AbstractDissimilarity diss, float[][] projection) {
        float error = 0.0f;
