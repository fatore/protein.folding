...
public class QuerySolver {
    ...
    public QuerySolver(Corpus corpus, ArrayList<AbstractInstance> instances) {
        this.corpus = corpus;
    ...
    protected float[] oper(String opr, float[] cdata1, float[] cdata2) {
        float[] cdata = new float[instances.size()];
    ...
    float[] value(String word) throws java.io.IOException {
        float[] cdata = new float[this.instances.size()];
        Arrays.fill(cdata, 0.0f);

        //Getting the Cdata
        for (int i = 0; i < instances.size(); i++) {
            cdata[i] = this.getWordFrequency(word, this.instances.get(i).getId());
        }
    ...
    public void createCdata(String query, Scalar scalar) throws java.io.IOException {
        ...
        for (int i = 0; i < cdata.length; i++) {
            if ((max - min) > 0.0 && (cdata[i] >= (numberOcurrences / instances.size()))) {
        ...
        for (int i = 0; i < instances.size(); i++) {
            AbstractInstance ai = instances.get(i);

            if (ai instanceof ProjectionInstance) {
                ((ProjectionInstance) ai).setScalarValue(scalar, cdata[i]);
            }
        }

    }

    private float getWordFrequency(String word, int id) throws IOException {
        return getWordFrequencyFromFile(word, id);
    }

    protected float getWordFrequencyFromFile(String word, int id) throws java.io.IOException {
        float frequency = 0.0f;

        ArrayList<Ngram> ngrams = this.corpus.getNgrams(id);
        for (Ngram n : ngrams) {
