...
private void generateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_generateButtonActionPerformed
    ...
    Thread t = new Thread() {
        ...
        public void run() {
          try {
                ...
                if (OpenDialog.checkCorpus((TopicProjection3DModel)model, ClusteringView.this.getParent())) {
                    AbstractTopic3DCreator topic = TopicFactory.getInstance((TopicProjection3DModel)model, TopicType.COVARIANCE);
                    ...
                    for (ArrayList<Integer> cluster : clusters) {
                        ...
                        ArrayList<TopicProjection3DInstance> clusterInstances2 = new ArrayList<TopicProjection3DInstance>(cluster.size());
                        for (Integer id : cluster) {
                            clusterInstances.add(instances.get(id));
                            clusterInstances2.add((TopicProjection3DInstance)instances.get(id));
                        }
                        Topic3D topicrender = topic.createTopic(clusterInstances2);
                        ...
                        ((DefaultTreeModel) tree.getModel()).insertNodeInto(new DefaultMutableTreeNode(topicrender),
                                currentNode, currentNode.getChildCount());

                        ((TopicProjection3DModel)model).addTopic(topicrender);
                    }
                }
    ...
    t.start();
    ClusteringView.this.setVisible(false);
