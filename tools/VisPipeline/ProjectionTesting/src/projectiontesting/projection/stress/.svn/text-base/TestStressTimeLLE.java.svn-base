 ...
 * @author PC
 */
public class TestStressTimeLLE extends TestStressTime {
    ...
    protected AbstractMatrix project(AbstractMatrix matrix, DissimilarityType dissstype) throws IOException {
        LLEProjectionComp lle = new LLEProjectionComp();
        lle.setDissimilarityType(dissstype);
        lle.setNumberNeighbors(nrneighbors);
        lle.input(matrix);
        lle.execute();
        AbstractMatrix projection = lle.output();
        return projection;
    }
