 ...
 */
public class TransformProjectionFrame extends ModelViewer {

    /** Creates new form ProjectionFrame */
    public TransformProjectionFrame() {
    ...
    private void finishButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_finishButtonActionPerformed
        if (model != null) {
            try {
                PLMPTransformProjection plmp = new PLMPTransformProjection();
                plmp.setFractionDelta(8.0f);
                plmp.setNumberIterations(100);
                plmp.setSampleMatrix(samplematrix);
                plmp.setSampleProjection(getRepositionedProjection());
                ...
                completemodel.output().changeColorScaleType(((TransformProjectionModel) model).getColorTable().getColorScaleType());
                ...
                //////////////////////////////////////
                ////starting the crazy part...
                //////////////////////////////////////

                //creting the projection matrix
                DenseDoubleMatrix2D proj = new DenseDoubleMatrix2D(projection.getRowCount(), projection.getDimensions());
                for (int i = 0; i < projection.getRowCount(); i++) {
                    proj.setQuick(i, 0, projection.getRow(i).getValue(0));
                    proj.setQuick(i, 1, projection.getRow(i).getValue(1));
                }

                //find the transformation inverse
                Algebra alg = new Algebra();
                DoubleMatrix2D inverse = alg.inverse(plmp.getTransformation());
                DoubleMatrix2D mult = alg.mult(proj, inverse);
                //System.out.println(mult);

                //creating the new transfomed matrix
                AbstractMatrix newmatrix = new DenseMatrix();
                for (int i = 0; i < mult.rows(); i++) {
                    float[] vect = new float[mult.columns()];

                    for (int j = 0; j < vect.length; j++) {
                        vect[j] = (float) mult.get(i, j);
                    }

                    newmatrix.addRow(new DenseVector(vect, matrix.getRow(i).getId(), matrix.getRow(i).getKlass()));
                }

                //projecting the transformed data to test
                ...
                idmap.input(newmatrix);
                idmap.execute();

                ProjectionModelComp transmodel = new ProjectionModelComp();
                transmodel.input(idmap.output());
                transmodel.execute();

                transmodel.output().changeColorScaleType(((TransformProjectionModel) model).getColorTable().getColorScaleType());

                ProjectionFrameComp frame2 = new ProjectionFrameComp();
                frame2.setTitle("Transformed Projection");
                frame2.input(transmodel.output());
                frame2.execute();
                ...
                Logger.getLogger(TransformProjectionFrame.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    private void initButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_initButtonActionPerformed
        try {
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
                ...
                //creating the model
                model = new TransformProjectionModel();
                ((TransformProjectionModel) model).changeColorScaleType(ColorScaleType.CATEGORY_SCALE);
                Scalar cdata = ((TransformProjectionModel) model).addScalar(ProjectionConstants.CDATA);
                int nrows = sampleproj.getRowCount();

                for (int i = 0; i < nrows; i++) {
                    AbstractVector row = sampleproj.getRow(i);
                    ProjectionInstance pi = new ProjectionInstance(((TransformProjectionModel) model), row.getId(),
    ...
    public class ViewPanel extends JPanel {
        ...
        public ViewPanel() {
            ...
            this.selection = new InstanceSelection(TransformProjectionFrame.this);
        }
    ...
    public static void main(String[] args) {
        //showing the frame
        TransformProjectionFrame frame = new TransformProjectionFrame();
